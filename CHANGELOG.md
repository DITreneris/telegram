...
- **Railway (verified deploy, 2026-04-06, us-east4):** **Railpack 0.23.0** build used **Python 3.13.12** (Railpack default) and **mise Node 22.22.1** from repo [`.nvmrc`](.nvmrc) during install; **`pip install -r requirements.txt`** and **`python run.py`** deploy completed successfully (~59s build). GitHub Actions **python** job still uses **Python 3.11**; for the same minor on Railway, set **`RAILPACK_PYTHON_VERSION=3.11`** (see [docs/RUNBOOK.md](docs/RUNBOOK.md)).
...
- **Post 35 hero PNG (`35_Prompt_Anatomy.png`):** [web/public/images/posts/35_Prompt_Anatomy.png](web/public/images/posts/35_Prompt_Anatomy.png) was a **1×1 placeholder** (~70 B) while [data/images/35_Prompt_Anatomy.png](data/images/35_Prompt_Anatomy.png) held the real asset; the queue manifest points at the **web** path, so delivery of **`post_035_photo`** (`/next` or scheduled jobs) could hit Telegram **`BadRequest: Image_process_failed`**. Replaced the web copy with the full image so it matches **`data/images`** (bot + web UI).
